...
std::string to_fen(const Mailbox& m)
{
    ...
    for (int row = 7; row >= 0; --row) {
        for (int col = 0; col < 8; ++col) {
            if (is_empty(m.square(Position{::Position{col, row}}))) {
                ++count;
            } else {
                push_back_count();
                result.push_back(to_char(m.square(Position{::Position{col, row}})));
            }
        }
        push_back_count();
        result.push_back('/');
...
Mailbox Mailbox::from_fen(const std::string& s)
{
    ...
    for (char ch : s) {
        ...
        if (ch > '0' && ch <= '9') {
            ...
            m.square_array[Position(::Position{col, row}).value()] = to_square(ch);
            ++col;
        }
...
void add_castling_moves(const Mailbox& m, Player player,
                        std::vector<Move>& moves)
{
    ...
    // Check Queenside
    [&] {
        if (!is_empty(m.square(king_initial_position.value() - 1))) return;
        if (!is_empty(m.square(king_initial_position.value() - 2))) return;
        if (!is_empty(m.square(king_initial_position.value() - 3))) return;

        const auto rook_square = m.square(king_initial_position.value() - 4);
        ...
        auto m_copy = m;
        m_copy.apply(Move{king_initial_position,
                          Position{king_initial_position.value() - 1}});
        if (in_check(m_copy, player)) return;

        moves.push_back(Move{king_initial_position,
                             Position{king_initial_position.value() - 2}});
    }();
    ...
    [&] {
        ...
        const auto rook_square = m.square(king_initial_position.value() + 3);
    ...
    for (int i = 0; i < m.size; ++i) {
        ...
        if (is_square_owned_by_player(square, player)) {
            const auto add_linear_moves = [&](int offset) {
                ...
                {
                    moves.push_back(Move{Position{i}, Position{destination}});
            ...
            case Piece::pawn: {
                ...
                if (is_empty(m.square(i + dir * 10))) {
                    moves.push_back(Move{Position{i}, Position{i + dir * 10}});
                    ...
                    {
                        moves.push_back(Move{Position{i}, Position{i + dir * 20}});
                    }
                }

                auto add_pawn_take_move = [&](int offset) {
                    const int destination = i + offset;
...
void Mailbox::apply(const Move& m)
{
    ...
    if (to_piece(square(m.begin)) == Piece::king &&
        std::abs(m.end.value() - m.begin.value()) == 2)
    {
        const bool kingside = m.end.value() > m.begin.value();

        auto rook_finishing_position = Position{
            (kingside ? 5 : 3), m.begin.row()};
        auto rook_starting_position  = Position{
            (kingside ? 7 : 0), m.begin.row()};

        auto& rook_square = square_array[rook_starting_position.value()];
        rook_square = remove_castling_rights(rook_square);

        this->apply(Move{rook_starting_position, rook_finishing_position});
    }

    auto& square_to_be_moved = square_array[m.begin.value()];
    square_to_be_moved = remove_castling_rights(square_to_be_moved);

    square_array[m.end.value()] = square(m.begin);
    square_array[m.begin.value()] = Square::empty;

    // Apply pawn promotion to queen
    if (to_piece(square_array[m.end.value()]) == Piece::pawn &&
        (m.end.row() == 0 || m.end.row() == 7)) {
            square_array[m.end.value()] = promote_to(square_array[m.end.value()], Piece::queen);
    }
}

bool in_check(const Mailbox& m, Player p)
{
    const int king_position = [&] {
        for (int i = 0; i < m.size; ++i) {
    ...
    if (king_position == -1) return false;

    auto moves = generate_moves(m, opponent_of(p));

    for (auto& move : moves) {
        if (move.end.value() == king_position) return true;
    }
    return false;
}
